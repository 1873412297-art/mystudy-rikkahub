...
- Implemented outcomes are organized on branch `codex/reintegrate-private-main-organized-2026-06-17`.
- The shared `service/group/GroupMessageContextFilter.kt` path is now the only group context filter implementation.
- Dynamic addressed-turn routing, moderator parsing, continuation-nudge cleanup, and runtime-state/event updates are present on the organized branch together with regression coverage.
- Branch-level verification passed after reintegration:
  - `./gradlew :app:compileDebugKotlin -x :web:buildWebUi`
  - `./gradlew :web:buildWebUi`
  - `./gradlew :app:assembleDebug`
- The checkbox tasks below remain useful as a granular execution template, but they were not re-checked step-by-step during the later history-organizing pass.
